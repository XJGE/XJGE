...
class ComponentArrow extends Component {
    ...
    static {
        texture = new Texture("image_arrow.png");
    ...
    void render(Map<String, GLProgram> glPrograms, Vector3f cameraPos) {
        ...
        graphics.modelMatrix.translation(target);
        graphics.modelMatrix.billboardCylindrical(target, cameraPos, up);
        graphics.modelMatrix.scale(0.75f);
        
        glProgram.setUniform("uTexture", 0);
        glProgram.setUniform("uColor", Color.WHITE.asVector3f());
    ...
    void target(Vector3f target) {
        this.target.set(target.x, target.y + 1.5f, target.z);
    }
