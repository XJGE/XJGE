...
class WidgetRules extends Widget {
    ...
    public void render(Map<String, GLProgram> glPrograms) {
        ...
        for(int i = 0; i < text.length; i++) {
            int xOffset = background.positionX + 10;
            int yOffset = background.positionY + background.height - (25 * (i + 1));
            
            switch(i) {
                case 0 -> Font.fallback.drawString("MINIGAME RULES:", xOffset, yOffset, Color.CYAN, 1f);
                case 1 -> Font.fallback.drawString(text[0], xOffset, yOffset, Color.SILVER, 1f);
                case 2 -> Font.fallback.drawString(text[1], xOffset, yOffset, Color.SILVER, 1f);
                case 3 -> Font.fallback.drawString(text[2], xOffset, yOffset, Color.SILVER, 1f);
                case 4 -> Font.fallback.drawString(text[3], xOffset, yOffset, Color.SILVER, 1f);
            }
        }
    }
    ...
    public final void relocate(SplitScreenType splitType, int viewportWidth, int viewportHeight) {
        ...
        background.positionX = viewportWidth - 410;
