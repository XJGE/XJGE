...
public class Model {
    ...
    public Model(String filename, int aiArgs) {
        ...
        try(InputStream file = Model.class.getResourceAsStream(filename)) {
    ...
    private void loadModel(InputStream file, int aiArgs) throws Exception {
        ...
        aiScene = aiImportFileEx((filename), aiArgs, aiFileIO);
     ...
     */
    public void render(Shader glProgram, Light[] lights, GLCaps capabilities, int shineValue, int depthTexHandle) {
     ...
     *                       shadow map or -1 if one has not been set
     */
    public void render(Shader glProgram, Light[] lights, int shineValue, int depthTexHandle) {
     ...
     */
    public void renderShadow(Shader depthProgram) {
        glEnable(GL_DEPTH_TEST);
        glEnable(GL_CULL_FACE);
        glCullFace(GL_FRONT);
     ...
     */
    public void renderShadow(Shader depthProgram, GLCaps capabiltites) {
        capabiltites.enable();
