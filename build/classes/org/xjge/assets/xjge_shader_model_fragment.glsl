...
void main() {
    ...
    if(uHasAlbedoMap == 1) albedo *= texture(uMaterial.albedoMap, ioUV).rgb;

    if(uHasMetallicMap == 1) {
        float m = texture(uMaterial.metallicMap, ioUV).r;
        metallic *= m;
    }

    if(uHasRoughnessMap == 1) {
        float r = texture(uMaterial.roughnessMap, ioUV).r;
        roughness *= r;
    ...
    if(uHasNormalMap == 1) {
        vec3 tangentNormal = texture(uMaterial.normalMap, ioUV).rgb * 2.0 - 1.0;
    ...
    for(int i = 0; i < uNumLights; i++) {
        ...
        //Radiance
        vec3 radiance = light.color_brightness.rgb * light.color_brightness.w * attenuation;
